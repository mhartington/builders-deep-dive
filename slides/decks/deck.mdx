...

{
  "builders": {
    "command-builder": {
      "implementation": "./src/command-builder/index",
      "schema": "./src/command-builder/schema.json",
      "description": "Run a shell command"
    }
  }
}
...
```typescript title="Returns the target, builder, and config" subtitle="{ project:string, target:string, configuration:string }"
export default createBuilder(extendExisitingBuilder);
...
export function extendBuilder(builder) {
  return (options: any, context: BuilderContext) => {
    return builder(options, context, {
      webpackConfiguration(config) {
        config.plugins.push(new NyanStyledProgressPlugin());
        return config;
      }
    });
  };
...
```ts subtitle="~400 LOC, plus imports and utils"  title="@angular-devkit/build-angular:browser"
