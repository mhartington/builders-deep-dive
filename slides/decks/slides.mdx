...
<iframe width="560" height="315" src="https://www.youtube-nocookie.com/embed/AG4OpNXFN4o?controls=0" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
...
```diff subtitle="A hack to work run to builder 🙈" 10
```
...
```diff subtitle="Get the options listed in angular.json" 13
```

```diff subtitle="Get the builder name we want to run" 14
```
...
```diff subtitle="Validates public options with internals" 18:21
```
...
```ts subtitle="819 LOC, plus imports and utils"  title="@angular-devkit/build-angular:browser"
