...
export function isConsoleLogServerMessage(m: any): m is ConsoleLogServerMessage {
  return (
    m && typeof m.category === 'string' && typeof m.type === 'string' && m.data && typeof m.data.length === 'number'
  );
}
...
export async function createConsoleLogServer(host: string, port: number): Promise<WebSocket.Server> {
  ...
  wss.on('connection', (ws) => {
    ws.on('message', (data) => {
      let msg: { category: string; type: string; data: any[] };
      ...
      if (msg.category === 'console') {
        ...
        msg.data = msg.data.map((d) => JSON.stringify(d, undefined, d?.length ? '' : '  '));
        if (status) {
